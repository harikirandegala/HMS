  ...
  return (
    <div className="space-y-8 animate-fade-in font-sans text-[#1E293B]">
      ...
      <div className="border-b border-slate-200 pb-5 flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="bg-white px-4 py-2 rounded-xl border border-slate-200 text-xs">
          <p className="font-semibold text-slate-600">Assigned Ward Hours</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-5 space-y-6">
              ...
              {appointments.length > 0 ? (
                appointments.map(apt => (
                  <div 
                    key={apt.id} 
                    id={`apt-card-${apt.id}`}
                    className="p-3 border.5 border-slate-100 rounded-xl bg-slate-50/50 hover:bg-slate-50 transition-all flex flex-col justify-between gap-2.5 relative pl-4 border-l-4 border-l-[#0D9488]"
                  >
                    <div>
                      ...
                      <p className="text-xs text-slate-500 mt-1 lines-clamp-2 italic">"{apt.reason}"</p>
                    </div>
                    ...
                    {apt.status === 'pending' && (
                      <div className="flex gap-2 justify-end pt-1">
                        <button
                          ...
                          className="text-[10px] bg-rose-50 text-rose-600 font-bold px-2.5 py-1 rounded-md hover:bg-rose-100 transition-colors"
                        >
                          Reject
                        </button>
                        <button
                          ...
                          className="text-[10px] bg-[#0D9488] text-white font-bold px-2.5 py-1 rounded-md hover:opacity-95 transition-opacity"
                        >
                          Accept/Confirm
                        </button>
                      </div>
                    ...
                    {apt.status === 'confirmed' && (
                      <div className="flex gap-2 justify-end pt-1">
                        <button
                          ...
                          className="text-[10px] bg-emerald-600 text-white font-bold px-2.5 py-1 rounded-md hover:bg-emerald-700 transition-colors"
                        >
                          Mark Completed
                        </button>
                      </div>
                ...
                <div id="no-apts" className="text-center py-6 text-xs italic text-slate-400">
          ...
          <div className="bg-white rounded-2xl border border-[#E2E8F0] shadow-sm overflow-hidden">
            <div className="p-4 border-b border-slate-100 bg-slate-50/50 flex gap-2 items-center">
              ...
              <input
                ...
                className="w-full text-xs bg-transparent border-none outline-none focus:ring-0 placeholder:text-slate-400"
              />
            </div>

            <div className="p-4 space-y-2 max-h-80 overflow-y-auto">
              <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">Hospital Records ({filteredPatients.length} Active Records)</h4>
              {filteredPatients.length > 0 ? (
                filteredPatients.map(pt => (
                  <button
                    ...
                    className={`w-full text-left p-3 rounded-xl border border-slate-100 transition-all flex items-center justify-between group ${
                      selectedPatientId === pt.id ? 'bg-[#0D9488]/5 border-[#0D9488]' : 'bg-white hover:bg-slate-50'
                    }`}
                  >
                    <div>
                      <h5 className="font-bold text-xs text-slate-900 group-hover:text-[#0D9488] transition-colors">{pt.fullName}</h5>
                      <p className="text-[10px] text-zinc-400 font-semibold font-mono mt-0.5">Insurance CardID: {pt.insuranceNo}</p>
                    </div>
                    <span className="text-[9px] font-semibold bg-slate-100 text-slate-500 py-0.5 px-2 rounded-md uppercase">View File</span>
                  </button>
                ))
              ) : (
                <div id="no-patients-match" className="text-center py-4 text-xs italic text-slate-400">
        ...
        <div className="lg:col-span-7">
          ...
          {selectedPatientId && selectedPatient ? (
            <div className="space-y-6">
              ...
              <div className="bg-white p-5 rounded-2xl border border-[#E2E8F0] shadow-sm relative overflow-hidden">
                <div className="absolute top-0 right-0 w-32 h-32 bg-[#0D9488]/5 rounded-bl-full pointer-events-none" />
                
                <span className="text-[10px] bg-[#0F172A] text-white px-2 py-0.5 rounded font-mono font-bold tracking-wider uppercase">
                  Active Clinical File
                </span>
                <h3 className="text-lg font-bold text-[#0F172A] mt-2">{selectedPatient.fullName}</h3>
                
                <div className="grid grid-cols-3 gap-4 mt-4 text-xs">
                  <div>
                    <span className="text-[10px] text-slate-400 uppercase font-semibold">Birth Register Date</span>
                    <p className="font-bold text-slate-800 mt-0.5">{selectedPatient.dob}</p>
                  </div>
                  <div>
                    <span className="text-[10px] text-slate-400 uppercase font-semibold">Registered Gender</span>
                    <p className="font-bold text-slate-800 mt-0.5">{selectedPatient.gender}</p>
                  </div>
                  <div>
                    <span className="text-[10px] text-slate-400 uppercase font-semibold">Hospital policy insurance</span>
                    <p className="font-bold text-slate-800 mt-0.5">{selectedPatient.insuranceNo}</p>
                  </div>
                </div>
                ...
                {selectedPatient.medicalHistorySummary && (
                  <div className="mt-4 pt-3 border-t border-slate-100 text-xs">
                    ...
                    <p className="text-slate-600 mt-0.5 leading-relaxed">{selectedPatient.medicalHistorySummary}</p>
                  </div>
                )}
              </div>
              ...
              <div className="bg-white rounded-2xl border border-[#E2E8F0] shadow-sm overflow-hidden p-6">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h3 className="font-bold text-base text-slate-900">Computerized Practitioner Clinical Pad & consult</h3>
                </div>

                {writeMessage && (
                  <div id="clinical-post-alert" className={`mb-4 p-2.5 text-xs font-semibold rounded-md ${
                    writeMessage.startsWith('success') ? 'bg-emerald-50 text-emerald-800 border-l-4 border-emerald-500' : 'bg-red-50 text-[#F43F5E] border-l-4 border-[#F43F5E]'
                  }`}>
                ...
                <form onSubmit={handleSubmitClinicalNote} className="space-y-4">
                  <div>
                    <label htmlFor="diag-input" className="block text-xs font-semibold text-slate-500 mb-1">Encounter Assessment Diagnosis</label>
                    <input
                      id="diag-input"
                      type="text"
                      ...
                      className="w-full text-xs py-2 px-3 border border-[#E2E8F0] rounded-xl focus:outline-none focus:border-[#0D9488]"
                      required
                    />
                  </div>

                  <div>
                    <label htmlFor="notes-input" className="block text-xs font-semibold text-slate-500 mb-1">Clinical Assessment Logs (Practitioner notes)</label>
                    <textarea
                      id="notes-input"
                      rows={3}
                      ...
                      className="w-full text-xs py-2 px-3 border border-[#E2E8F0] rounded-xl focus:outline-none focus:border-[#0D9488] resize-none"
                      required
                    />
                  </div>
                  ...
                  <div className="bg-slate-50/50 p-4 rounded-xl border border-slate-100">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
                      <div>
                        <label htmlFor="vital-bp" className="block text-[10px] font-semibold text-zinc-500 mb-0.5">BP (mmHg)</label>
                        <input
                          id="vital-bp"
                          type="text"
                          value={bloodPressure}
                      ...
                      <div>
                        <label htmlFor="vital-hr" className="block text-[10px] font-semibold text-zinc-500 mb-0.5">Heart Rate (BPM)</label>
                        <input
                          id="vital-hr"
                          type="text"
                          value={heartRate}
                      ...
                      <div>
                        <label htmlFor="vital-temp" className="block text-[10px] font-semibold text-zinc-500 mb-0.5">Temp (°C)</label>
                        <input
                          id="vital-temp"
                          type="text"
                          value={temperature}
                      ...
                      <div>
                        <label htmlFor="vital-weight" className="block text-[10px] font-semibold text-zinc-500 mb-0.5">Weight (kg)</label>
                        <input
                          id="vital-weight"
                          type="text"
                          value={weight}
                  ...
                  <div className="bg-emerald-500/5 p-4 rounded-xl border border-[#0D9488]/10">
                    ...
                    <div className="flex gap-2 mb-3">
                      <input
                        ...
                        className="flex-1 text-xs px-3 py-1.5 border border-[#E2E8F0] bg-white rounded-lg focus:outline-none"
                      ...
                      <button
                        ...
                        className="bg-[#0f172a] hover:bg-slate-800 text-white font-bold text-xs px-3 py-1.5 rounded-lg flex items-center gap-1"
                      >
                        Insert Rx
                      </button>
                    </div>
                    ...
                    {prescriptionsArrayList.length > 0 ? (
                      <div className="space-y-1.5 bg-white p-2.5 rounded-lg border border-slate-200">
                        {prescriptionsArrayList.map((rx, index) => (
                          <div key={index} className="flex justify-between items-center text-xs text-slate-700 bg-slate-50 p-1.5 rounded font-mono">
                            <span>💊 {rx}</span>
                            <button
                              type="button"
                      ...
                      <p className="text-[10px] text-slate-400 italic">No medicinal chemical compounds prescribed.</p>
                    )}
                  </div>
                  ...
                  <button
                    ...
                    type="submit"
                    className="w-full py-2.5 bg-[#0D9488] hover:bg-[#0c1e26] text-white font-bold text-sm rounded-xl transition-colors flex items-center justify-center gap-1.5"
                  >
              ...
              <div className="bg-white rounded-2xl border border-[#E2E8F0] shadow-sm p-6 space-y-4">
                <h4 className="font-bold text-sm text-[#0F172A] border-b border-slate-100 pb-2">CHRONOLOGICAL CLINICAL FILE JOURNAL TIMELINE</h4>
                  ...
                  {patientRecords.length > 0 ? (
                    patientRecords.map((rec, idx) => (
                      <div key={rec.id} id={`medical-timeline-item-${idx}`} className="relative">
                        {/* Timeline dot */}
                        <div className="absolute -left-6 top-1.5 w-3 h-3 bg-[#0D9488] rounded-full border border-white" />
                        
                        <div className="p-4 bg-slate-50/50 rounded-xl border border-slate-100">
                          <div className="flex justify-between items-start text-xs text-slate-400 mb-1">
                            <span className="font-bold text-slate-700">📆 Visited: {rec.date}</span>
                            <span className="italic">Dr: {rec.doctorName}</span>
                          </div>
                          
                          <h5 className="font-bold text-xs text-slate-900 mt-1 uppercase">Diagnosis: {rec.diagnosis}</h5>
                          <p className="text-xs text-slate-600 mt-1 whitespace-pre-wrap leading-relaxed">"{rec.notes}"</p>

                          {rec.vitals && (
                            <div className="mt-3 grid grid-cols-2 md:grid-cols-4 gap-1.5 border-t border-slate-100 pt-2 text-[10px] text-slate-500 font-mono">
                          ...
                          {rec.prescriptions && rec.prescriptions.length > 0 && (
                            <div className="mt-2 text-[11px] bg-white p-2 rounded border border-slate-200/60">
                              <p className="text-[10px] text-indigo-600 font-bold uppercase mb-1">Assigned Prescriptions Rx:</p>
                              <div className="space-y-1">
                                {rec.prescriptions.map((px, rxIdx) => (
                                  <div key={rxIdx} className="text-slate-700 font-mono">
                                    • {px}
                                  </div>
                                ))}
                              </div>
                    ...
                    <div id="no-history-logs" className="py-2 text-xs italic text-slate-400 pl-2">
            ...
            <div id="pt-selector-placeholder" className="bg-white p-12 rounded-2xl border border-[#E2E8F0] text-center max-w-sm mx-auto shadow-sm">
              <Clipboard className="h-12 w-12 text-slate-300 mx-auto stroke-[1.5] mb-3" />
              <h3 className="font-sans font-bold text-[#0F172A] text-sm">Select Inpatient Medical Record</h3>
