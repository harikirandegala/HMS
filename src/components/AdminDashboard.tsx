...
export default function AdminDashboard({ token, formatPrice, currency, activeTab }: AdminDashboardProps) {
    ...
    <div className="space-y-8 animate-fade-in font-sans text-[#1E293B] dark:text-slate-100">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-5">
        <div>
          ...
          <h2 className="text-2xl font-bold tracking-tight text-[#0F172A] dark:text-white mt-1.5">
      ...
      {(showAll || showAppointments || showBilling) && (
        <div className={showAll ? "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6" : "max-w-md mx-auto w-full"}>
          ...
          {showAll && (
            <div className="bg-white dark:bg-[#111827] p-6 rounded-2xl border border-[#E2E8F0] dark:border-[#1F2937] shadow-sm hover:shadow-md transition-shadow">
              ...
              <div className="mt-4">
                ...
                <p className="text-xs text-slate-400 dark:text-slate-500 mt-1 font-semibold uppercase tracking-wider">Registered Patients</p>
              </div>
            </div>
          )}
          ...
          {showAll && (
            <div className="bg-white dark:bg-[#111827] p-6 rounded-2xl border border-[#E2E8F0] dark:border-[#1F2937] shadow-sm hover:shadow-md transition-shadow">
              ...
              <div className="mt-4">
                ...
                <p className="text-xs text-slate-400 dark:text-slate-500 mt-1 font-semibold uppercase tracking-wider">Active Physicians</p>
              </div>
            </div>
          )}
          ...
          {showAppointments && (
            <div className="bg-white dark:bg-[#111827] p-6 rounded-2xl border border-[#E2E8F0] dark:border-[#1F2937] shadow-sm hover:shadow-md transition-shadow">
              ...
              <div className="mt-4">
                ...
                <p className="text-xs text-slate-400 dark:text-slate-500 mt-1 font-semibold uppercase tracking-wider">Intake Sessions Today</p>
              </div>
            </div>
          )}
          ...
          {showBilling && (
            <div className="bg-white dark:bg-[#111827] p-6 rounded-2xl border border-[#E2E8F0] dark:border-[#1F2937] shadow-sm hover:shadow-md transition-shadow">
              ...
              <div className="mt-4">
                ...
                <p className="text-xs text-slate-400 dark:text-slate-500 mt-1 font-semibold uppercase tracking-wider">Accumulated Revenue</p>
              </div>
            </div>
          )}
      ...
      {/* Visual Analytics Block - Recharts */}
      {(showAll || showAppointments || showBilling) && (
        <div className={showAll ? "grid grid-cols-1 lg:grid-cols-3 gap-6" : "max-w-3xl mx-auto w-full"}>
          
          {/* Left Trend Area Chart */}
          {showBilling && (
            <div className={showAll ? "bg-white dark:bg-[#111827] p-5 rounded-2xl border border-[#E2E8F0] dark:border-[#1F2937] shadow-sm lg:col-span-2" : "bg-white dark:bg-[#111827] p-5 rounded-2xl border border-[#E2E8F0] dark:border-[#1F2937] shadow-sm"}>
      ...
      {(showAll || showBilling) && (
        <div className={showAll ? "grid grid-cols-1 lg:grid-cols-2 gap-6" : "max-w-3xl mx-auto w-full"}>

          {/* Invoicing Section */}
          {showBilling && (
            <div className="bg-white dark:bg-[#111827] p-6 rounded-2xl border border-[#E2E8F0] dark:border-[#1F2937] shadow-sm flex flex-col justify-between">
              <div>
                ...
                <form onSubmit={handleInvoiceSubmit} className="space-y-4">
                  <div>
                    <label htmlFor="inv-patient" className="block text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1">Select Hospitalized Patient</label>
                    <select
                      id="inv-patient"
                      value={invPatientId}
                    ...
                    >
                      ...
                      {patients.map(p => (
                        <option key={p.id} value={p.id} className="dark:bg-[#111827]">{p.fullName} ({p.insuranceNo})</option>
                      ))}
                    </select>
                  </div>

                  <div>
                    <label htmlFor="inv-desc" className="block text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1">Billing Item / Treatment Description</label>
                    <input
                      id="inv-desc"
                      type="text"
                  ...
                  <button
                    ...
                    type="submit"
                    className="w-full py-2 bg-[#0F172A] dark:bg-[#0D9488] hover:bg-slate-800 dark:hover:bg-[#0b7e73] text-white font-semibold text-sm rounded-xl transition-colors flex items-center justify-center gap-1.5 cursor-pointer"
                  >
